export const idols = [
  {
    id: 1,
    name: 'Jungkook',
    group: 'BTS',
    image: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRI8UwNzPIwVqvBvYGuvphwlIMMr6CQue38GQ&s',
    favoriteFoods: [
      {
        _id: 'idol-1-1',
        name: 'Korean BBQ Beef',
        description: 'Premium grilled beef with traditional marinades',
        price: 24.99,
        image: 'https://images.unsplash.com/photo-1590301157890-4810ed352733?w=400',
        category: 'Korean BBQ'
      },
      {
        _id: 'idol-1-2',
        name: 'Tteokbokki',
        description: 'Spicy rice cakes in gochujang sauce',
        price: 12.99,
        image: 'https://images.unsplash.com/photo-1583224994076-60ef8a4d4a1b?w=400',
        category: 'Street Food'
      },
      {
        _id: 'idol-1-3',
        name: 'Gimbap',
        description: 'Korean sushi rolls with various fillings',
        price: 8.99,
        image: 'https://images.unsplash.com/photo-1617196034796-73dfa7b1fd56?w=400',
        category: 'Street Food'
      }
    ]
  },
  {
    id: 2,
    name: 'Lisa',
    group: 'BLACKPINK',
    image: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSogJVKyVeRwoGCMm-g2P-JYfKHe3yqtAlZfQ&s',
    favoriteFoods: [
      {
        _id: 'idol-2-1',
        name: 'Tom Yum Soup',
        description: 'Spicy and sour Thai soup with shrimp',
        price: 18.99,
        image: 'https://images.unsplash.com/photo-1548943487-a2e4e43b4853?w=400',
        category: 'Soup'
      },
      {
        _id: 'idol-2-2',
        name: 'Pad Thai',
        description: 'Stir-fried rice noodles with peanuts',
        price: 14.99,
        image: 'https://images.unsplash.com/photo-1559314809-0d155014e29e?w=400',
        category: 'Thai'
      },
      {
        _id: 'idol-2-3',
        name: 'Mango Sticky Rice',
        description: 'Sweet coconut rice with fresh mango',
        price: 9.99,
        image: 'https://images.unsplash.com/photo-1621293954908-907159247fc8?w=400',
        category: 'Dessert'
      }
    ]
  },
  {
    id: 3,
    name: 'IU',
    group: 'Solo Artist',
    image: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQ2apvwTbsa22yr17v8eq8iH6Q7lRcukkBUlQ&s',
    favoriteFoods: [
      {
        _id: 'idol-3-1',
        name: 'Bibimbap',
        description: 'Mixed rice bowl with vegetables and egg',
        price: 15.99,
        image: 'https://images.unsplash.com/photo-1553163147-622ab57be1c7?w=400',
        category: 'Rice Bowl'
      },
      {
        _id: 'idol-3-2',
        name: 'Korean Fried Chicken',
        description: 'Crispy fried chicken with sweet sauce',
        price: 16.99,
        image: 'https://images.unsplash.com/photo-1575932444877-5106bee2a599?w=400',
        category: 'Fried Chicken'
      },
      {
        _id: 'idol-3-3',
        name: 'Mandu',
        description: 'Korean dumplings with pork filling',
        price: 10.99,
        image: 'https://images.unsplash.com/photo-1496116218417-1a781b1c416c?w=400',
        category: 'Dumplings'
      }
    ]
  },
  {
    id: 4,
    name: 'Kai',
    group: 'EXO',
    image: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQTzuAUgO56S5wX03Yk3KiJETI4WhCqmTgP0A&s',
    favoriteFoods: [
      {
        _id: 'idol-4-1',
        name: 'Japchae',
        description: 'Stir-fried glass noodles with vegetables',
        price: 14.99,
        image: 'https://images.unsplash.com/photo-1603133872878-684f208fb84b?w=400',
        category: 'Noodles'
      },
      {
        _id: 'idol-4-2',
        name: 'Ssam',
        description: 'Wrapped pork belly with lettuce',
        price: 22.99,
        image: 'https://images.unsplash.com/photo-1529006557810-274b9b2fc783?w=400',
        category: 'Korean BBQ'
      },
      {
        _id: 'idol-4-3',
        name: 'Haemul Pajeon',
        description: 'Seafood pancake with green onions',
        price: 13.99,
        image: 'https://images.unsplash.com/photo-1607532941433-304659e8198a?w=400',
        category: 'Pancake'
      }
    ]
  },
  {
    id: 5,
    name: 'V',
    group: 'BTS',
    image: 'https://i.pinimg.com/originals/24/9c/08/249c08fdbe66d8e5dd28e28b01c42518.jpg',
    favoriteFoods: [
      {
        _id: 'idol-5-1',
        name: 'Jjajangmyeon',
        description: 'Black bean noodles',
        price: 11.99,
        image: 'https://images.unsplash.com/photo-1583224964978-2257b960c3d3?w=400',
        category: 'Noodles'
      },
      {
        _id: 'idol-5-2',
        name: 'Samgyeopsal',
        description: 'Grilled pork belly',
        price: 19.99,
        image: 'https://images.unsplash.com/photo-1590301157890-4810ed352733?w=400',
        category: 'Korean BBQ'
      },
      {
        _id: 'idol-5-3',
        name: 'Patbingsu',
        description: 'Shaved ice with red beans',
        price: 8.99,
        image: 'https://images.unsplash.com/photo-1501443762994-82bd5dace89a?w=400',
        category: 'Dessert'
      }
    ]
  },
  {
    id: 6,
    name: 'Rosé',
    group: 'BLACKPINK',
    image: 'https://i.pinimg.com/736x/72/25/be/7225be2318447404c6186d7f259cde74.jpg',
    favoriteFoods: [
      {
        _id: 'idol-6-1',
        name: 'Kimbap',
        description: 'Korean sushi rolls',
        price: 8.99,
        image: 'https://images.unsplash.com/photo-1617196034796-73dfa7b1fd56?w=400',
        category: 'Street Food'
      },
      {
        _id: 'idol-6-2',
        name: 'Jajangmyeon',
        description: 'Black bean noodles',
        price: 11.99,
        image: 'https://images.unsplash.com/photo-1583224964978-2257b960c3d3?w=400',
        category: 'Noodles'
      },
      {
        _id: 'idol-6-3',
        name: 'Strawberry Cake',
        description: 'Fresh strawberry cream cake',
        price: 12.99,
        image: 'https://images.unsplash.com/photo-1565958011703-44f9829ba187?w=400',
        category: 'Dessert'
      }
    ]
  }
];

export default idols;
